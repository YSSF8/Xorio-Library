...
export function count(parent, seconds) {
    ...
    if (Number.isInteger(seconds) == false) {
        clearInterval(loop);
        count.remove();
        throw new Error('Error: Value can only be an "integer" \n   Try removing the quotes from the "value" if it\'s string');
    }

    // Checks if the seconds is null, if it's null it will give you an error and it will not work
    if (seconds == null) {
        throw new Error(`Error: Null number \nTry using this: \n   xorio.count("${parent}", 0);`);
    }
}
...
export function alert(title, buttonText) {
    ...
    function err() {
        ...
        throw new Error(`Error: Title can not be null \nTry using this: \n   xorio.alert("${document.title}");`);
    }
...
export class Random {
    ...
    number(parent) {
        ...
        randElement.innerHTML = Math.floor(Math.random() * 10);
        randElement.style.fontFamily = 'Segoe UI';
    }
}